...
import time
from Model_define_pytorch import NMSELoss, FC_ELU_Estimation,CE_ResNet18,SD_BiLSTM
...
if args.load_SD:
    SD_path = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/XDH2X_' + args.model + '_SD_mode'+str(mode)+'_Pilot'+str(Pilot_num)+'.pth.tar'                
...
for epoch in range(epochs):
    ...
    for it, (Y_train, X_train, H_train) in enumerate(train_dataloader):
        ...
        input3 = torch.cat([X_input_train.cuda(), Yd_input_train.cuda(), H_train_padding], 2)

        input3 = input3.reshape(batch_size, 2*8, 256)
        input3 = input3.permute(2,0,1).contiguous()
        # 第三层网络的输出
        output3 = SD(input3)
    ...
    #         input3 = torch.cat([X_input_test.cuda(), Yd_input_test.cuda(), H_test_padding], 2)
    
    #         input3 = input3.reshape(Ns, 2*8, 256)
    #         input3 = input3.permute(2,0,1).contiguous()
    #         # 第三层网络的输出
    #         output3 = SD(input3)
    ...
    #         error_SD = np.abs(output3 - label)
    #         average_accuracy_SD = np.sum(error_SD < eps) / error_SD.size



    #         # print('ML Accuracy:%.4f' % average_accuracy_ML, 'SD Accuracy:%.4f' % average_accuracy_SD)
    #         print('SD Accuracy:%.4f' % average_accuracy_SD)
    #         if average_accuracy_SD > best_accuracy:
    #             # model save
                
    #             SDSave = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/XDH2X_' + args.model + '_SD_mode'+str(mode)+'_Pilot'+str(Pilot_num)+'.pth.tar'                
    ...
    #             best_accuracy = average_accuracy_SD
