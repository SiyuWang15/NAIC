...
def get_YH_data_random(mode, Pn):
    ...
    H_train = np.load(H_path)

    H_path = os.path.join(dataset_prefix, 'dataset/H_val.bin')
    H_val = open(H_path, 'rb')
    H_val = struct.unpack('f'*2*2*2*32*2000, H_val.read(4*2*2*2*32*2000))
    H_val = np.reshape(H_val, [2000, 2, 4, 32]).astype('float32')
    H_val = H_val[:, 1, :, :] + 1j*H_val[:, 0, :, :]
