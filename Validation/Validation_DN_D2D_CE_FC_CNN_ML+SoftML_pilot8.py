...
import time
from Model_define_pytorch import NMSELoss, ResNet18, ResNet34,ResNet50,U_Net,FC_ELU_Estimation,CE_ResNet18,D2D_U_Net
...
DN = D2D_U_Net()
path_DN = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/D2D_Unet_Filter_mode'+str(mode)+'_Pilot'+str(Pilot_num)+'.pth.tar'
...
with torch.no_grad():
    ...
    #降噪  
    input3 = Yd_input_test.contiguous()
    output = DN(input3)
    ...
    print(criterion_DN(input3, Yd_input_test_woN))
    print(criterion_DN(output, Yd_input_test_woN))
    # ML性能对比
    ...
    for snr in range(5,10):
        ...
        error_SoftML = np.abs(X_Softbits - X_test.numpy())
        accuracy_SoftML = np.sum(error_SoftML < eps)/error_SoftML.size
        print('SNR = ', snr, 'After Filter：SoftML accuracy %.4f' % accuracy_SoftML)
