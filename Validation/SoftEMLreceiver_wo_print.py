...
def SoftEMLReceiver_single_process(q, label, Y, H, Edh, Rdh, SNRdb = 10):
    # 输入：
    ...
    for num in range(batch):
        for idx in range(256):
            ...
            for idx1 in range(2**G):
                ...
                error[idx1] = np.abs(((y - Ehx).T.conj()).dot(np.linalg.inv(Chx)).dot(y - Ehx))  
            # 软bit计算
            for idx2 in range(G):
                #依靠codebook检索
                ...
                LR0 = np.sum(np.exp( - error[ code_idx < 0.5 ] ))
                LR1 = np.sum(np.exp( - error[ code_idx > 0.5 ] ))
                if LR0 > LR1:
                    x_bit[idx2] = 0.
                else:
                    x_bit[idx2] = 1.
...
def SoftEMLReceiver(Y, H, Edh, Rdh, Codebook = MakeCodebook(4), SNRdb = 10, num_workers = 16):
    # 输入：
    # Y : frequency domain 复数 (batch * 2 * 256) [样本数 * 天线数 * 子载波数]
    # H : frequency domain 复数 (batch * 2 * 2 * 256) [样本数 * 接收天线数 * 发射天线数 * 子载波数]
    # 输出：
    ...
    for i in range(num_workers):
        if i < num_workers - 1:
            ...
            P = mp.Process( target = SoftEMLReceiver_single_process, args = (q, i, Y_single, H_single, Edh, Rdh, SNRdb ))
            # P.start()
            Processes.append(P)
        else:
            Y_single = Y[  i*batch :, ...]
