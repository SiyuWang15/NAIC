...
from SoftEMLreceiver_wo_print import SoftEMLReceiver, getERH
from MLreceiver_wo_print import MLReceiver
...
with torch.no_grad():
    ...
    X_SoftML, X_SoftMLbits = SoftMLReceiver(Yd, Hf, SNRdb = 5)

    error_SoftML = np.abs(X_SoftMLbits - X_test.numpy())
    accuracy_SoftML = np.sum(error_SoftML < eps)/error_SoftML.size
    print('SNR=',5,'SoftML accuracy %.4f' % accuracy_SoftML)

    # for snr in range(4,10):
    #     X_SoftML, X_SoftMLbits = SoftMLReceiver(Yd, Hf, SNRdb = snr)

    #     error_SoftML = np.abs(X_SoftMLbits - X_test.numpy())
    #     accuracy_SoftML = np.sum(error_SoftML < eps)/error_SoftML.size
    #     print('SNR=',snr,'SoftML accuracy %.4f' % accuracy_SoftML)



    hf_hat = Hf
    hf_true = np.reshape(Hf_test, [-1,2,2,256], order = 'F')
    # Edh, Rdh = getERH(hf_hat, hf_true)
    Edh = np.load('Edh.npy')
    Rdh = np.load('Rdh.npy')
    X_SoftEML, X_SoftEMLbits = SoftEMLReceiver(Yd, Hf, Edh, Rdh, SNRdb = 10)

    error_SoftEML = np.abs(X_SoftEMLbits - X_test.numpy())
    accuracy_SoftEML = np.sum(error_SoftEML < eps)/error_SoftEML.size
    print('SNR=',10,'SoftEML accuracy %.4f' % accuracy_SoftEML)


    # for snr in range(6,12):
    #     X_SoftEML, X_SoftEMLbits = SoftEMLReceiver(Yd, Hf, Edh, Rdh, SNRdb = snr)

    #     error_SoftEML = np.abs(X_SoftEMLbits - X_test.numpy())
    #     accuracy_SoftEML = np.sum(error_SoftEML < eps)/error_SoftEML.size
    #     print('SNR=',snr,'SoftEML accuracy %.4f' % accuracy_SoftEML)
