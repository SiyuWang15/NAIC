...
import time
from Model_define_pytorch import NMSELoss, FC_ELU_Estimation,CE_ResNet34,XYH2X_ResNet18,XDH2H_Resnet,densenet121
...
SD_path = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/HS_SD_Pilot8' 
SD = pD.DeepRx(SD_path,cuda_gpu=True,gpus=[0,1])    
print("Weight For SD2 Loaded!")

CE2 = XDH2H_Resnet()
'''
CE2_path = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/CE2_0.14927.pth'                
...
if Pilot_num == 32:
    ...
    X_1.tofile('./X_pre_1_CE_FC_Resnet34_SD_HS_CE2_Resnet34_NHSCE_' + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) +'_N_'+str(N)+ '.bin')


if Pilot_num == 8:
    X_1 = np.array(np.floor(X_bits + 0.5), dtype=np.bool)
    X_1.tofile('./X_pre_2_CE_FC_Resnet34_SD_HS_CE2_Resnet34_NHSCE_' + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) +'_N_'+str(N)+ '.bin')
