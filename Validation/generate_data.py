...
def generatorXY_DN(batch, H, Pilot_num,SNR,m):
    input_labels = []
    input_samples = []
    input_channels = []
    input_samples_woN = []
    for row in range(0, batch):
        ...
        # print(SNRdb, mode)
        YY = MIMO(X, HH, SNRdb, mode, Pilot_num) / 20  ###
        YY_woN = MIMO(X, HH, 100, mode, Pilot_num) / 20  ###
        XX = np.concatenate((bits0, bits1), 0)
        input_labels.append(XX)
        input_samples.append(YY)
        input_samples_woN.append(YY_woN)
        input_channels.append(HH)

    batch_y = np.asarray(input_samples)
    batch_x = np.asarray(input_labels)
    batch_h = np.asarray(input_channels)
    batch_y_woN = np.asarray(input_samples_woN)
    return batch_y, batch_x, batch_h,batch_y_woN
...
class RandomDataset_DN():
    ...
    def __getitem__(self, index):
        ...
        if self.mode == -1:
            mm = np.random.randint(0, 3)
        YY = MIMO([bits0, bits1], HH, SS, mm, self.Pilot_num)/20
        YY_woN = MIMO([bits0, bits1], HH, 100, mm, self.Pilot_num)/20
        XX = np.concatenate([bits0, bits1], 0)
        return YY, XX, HH, YY_woN
