...
class DenseNet_BC(nn.Module):
    def __init__(self, growth_rate=12, block_config=(6,12,24,16),
                 bn_size=4, theta=0.5, num_classes=1024):
        ...
        self.classifier = nn.Linear(num_feature * 2 * 2, num_classes)
    ...
    def forward(self, x):
        ...
        out = out.view(out.size(0), 1024)
        return torch.sigmoid(out)


# DenseNet_BC for ImageNet
def DenseNet121():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 24, 16), num_classes=1024)

def DenseNet169():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 32, 32), num_classes=1024)

def DenseNet201():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 48, 32), num_classes=1024)

def DenseNet161():
    return DenseNet_BC(growth_rate=48, block_config=(6, 12, 36, 24), num_classes=1024)

def DenseNet100():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 24), num_classes=1024)



if __name__ == '__main__':
    input = torch.randn(256, 1, 20, 256)
    model = DenseNet201()
