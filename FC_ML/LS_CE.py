...
if __name__ == '__main__':
    ...
    Hfper = np.zeros(shape=(Ns, 2, 2, 32), dtype=np.complex64)
    Hfper[:, :, 0, :] = Hf[:, :, 0, pilotCarriers1]
    Hfper[:, :, 1, :] = Hf[:, :, 1, pilotCarriers2]

    Hfhat1 = LS_Estimation_Partial(y,Pilotnum)
    # Hfhat2 = MMSE_Estimation(y,Pilotnum)

    print(np.sum(abs(Hfper)**2))

    nmse1 = np.sum(abs(Hfhat1-Hfper)**2)/np.sum(abs(Hfper)**2)
    print(nmse1)
    #
