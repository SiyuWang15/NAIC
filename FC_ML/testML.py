...
if __name__ == '__main__':
    ...
    X_ML2, X_bits2 = SoftMLReceiver(output_data, Hf)
    error_X2 = input - X_ML2
    error_X_bit2 = X - X_bits2

    print('Complex Accuracy: ', np.sum(np.abs(error_X2)<0.1)/np.size(error_X2)*100,'%')
    print('Bits Accuracy: ', np.sum(np.abs(error_X_bit2)<0.1)/np.size(error_X_bit2)*100,'%')
