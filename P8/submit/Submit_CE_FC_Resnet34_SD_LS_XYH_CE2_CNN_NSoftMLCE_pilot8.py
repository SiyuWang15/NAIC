...
def run(prefix = './', gpu_list = '6,7', N = 3):
    ...
    if Pilot_num == 32:
        ...
        X_1.tofile(prefix + 'results/X_pre_1_CE_FC_Resnet34_SD_LS_XYH_CE2_Resnet34_NSoftMLCE_N_'+str(N)+ '.bin')


    if Pilot_num == 8:
        X_1 = np.array(np.floor(X_bits + 0.5), dtype=np.bool)
        X_1.tofile(prefix + 'results/X_pre_2_CE_FC_Resnet34_SD_LS_XYH_CE2_Resnet34_NSoftMLCE_N_'+str(N)+ '.bin')
