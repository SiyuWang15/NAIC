...
def run(prefix = './', gpu_list = '6,7', N = 3):
    ...
    CE2_path = prefix + 'checkpoints/CE2_SD_0.14486.pth'  
    ...
    if Pilot_num == 32:
        ...
        X_1.tofile(prefix + 'results/X_pre_1_CE_FC_CNN_SD_HS_CE2_Resnet34_NHSCE_N_'+str(N)+ '.bin')


    if Pilot_num == 8:
        X_1 = np.array(np.floor(X_bits + 0.5), dtype=np.bool)
        X_1.tofile(prefix + 'results/X_pre_2_CE_FC_CNN_SD_HS_CE2_Resnet34_NHSCE_N_'+str(N)+ '.bin')
