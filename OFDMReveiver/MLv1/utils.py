...
def PS(bits):
    ...
    #signal_output00, para = ofdm_simulate(bits0(512 bit), 
    # HMIMO[0,:] dim 32*1
    # SNRdb = 10dB
    # mu = 2 (QAM)
    # cpflag = 1
...
def ofdm_simulate(codeword, channelResponse, SNRdb, mu, CP_flag, K, P, CP, pilotValue, pilotCarriers, dataCarriers,
    ...
    codeword_qam = Modulation(codeword, mu) # dim 256
    ...
    AA = np.concatenate((np.real(OFDM_RX_noCP), np.imag(OFDM_RX_noCP))) # 512dim
...
#  P = 8 | 32
# the number of bits : P*2(QAM modulation)*2(MIMO)
def MIMO(X, HMIMO, SNRdb,flag,P):
    ...
    pilotValue = Modulation(bits, mu) # 64 * 1
    ...
    bits0=X[0] # shape 128*4
    bits1=X[1]
    pilotCarriers1 = pilotCarriers[0:P:2] # 32 dim perhaps for antenna 1 
    pilotCarriers2 = pilotCarriers[1:P:2] # 32 dim perhaps for antenna 2

    ## sequently calculate each antenna
