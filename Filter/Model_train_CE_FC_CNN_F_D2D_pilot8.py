...
import time
from Model_define_pytorch import NMSELoss, FC_ELU_Estimation,CE_ResNet18,D2D_U_Net
...
if args.load_Filter:
    Filter_path = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/D2D_' + args.model + '_Filter_mode'+str(mode)+'_Pilot'+str(Pilot_num)+'.pth.tar'                
...
for epoch in range(epochs):
    ...
    for it, (Y_train, X_train, H_train, Y_woN_train) in enumerate(train_dataloader):
        ...
        input = Yd_input_train.cuda()
        label = Yd_input_label.cuda()
        # 第三层网络的输出
        output = Filter(input)
    ...
    with torch.no_grad():
        ...
        for Y_test, X_test, H_test,Y_woN_test in test_dataloader:
            ...
            # 计算loss
            nmse = criterion_Filter(input, label).detach().cpu().numpy()
            # print('ML Accuracy:%.4f' % average_accuracy_ML, 'Filter Accuracy:%.4f' % average_accuracy_Filter)
            print('Before Filter NMSE:%.4f' % nmse)

            # 计算loss
            nmse = criterion_Filter(output, label).detach().cpu().numpy()
            # print('ML Accuracy:%.4f' % average_accuracy_ML, 'Filter Accuracy:%.4f' % average_accuracy_Filter)
            print('After Filter NMSE:%.4f' % nmse)
            if nmse < best_nmse:
                # model save
                
                FilterSave = '/data/CuiMingyao/AI_competition/OFDMReceiver/Modelsave/D2D_' + args.model + '_Filter_mode'+str(mode)+'_Pilot'+str(Pilot_num)+'.pth.tar'                
