...
def run(prefix = './', gpu_list = '6,7', N = 3):
    ...
    path = prefix + 'checkpoints/resnet34.pth'
    ...
    SD_path = prefix + 'checkpoints/HS_SD_Pilot8' 
    SD = pD.DeepRx(SD_path,cuda_gpu=True,gpus=[0,1])    
    print("Weight For SD2 Loaded!")

    CE2 = XDH2H_Resnet()
    CE2_path = prefix + 'checkpoints/CE_SD_Resnet34_0.15032.pth'  
    ...
    if Pilot_num == 32:
        ...
        X_1.tofile(prefix + 'results/X_pre_1_CE_FC_Resnet34_SD_HS_CE2_Resnet34_NHSCE_N_'+str(N)+ '.bin')


    if Pilot_num == 8:
        X_1 = np.array(np.floor(X_bits + 0.5), dtype=np.bool)
        X_1.tofile(prefix + 'results/X_pre_2_CE_FC_Resnet34_SD_HS_CE2_Resnet34_NHSCE_N_'+str(N)+ '.bin')
